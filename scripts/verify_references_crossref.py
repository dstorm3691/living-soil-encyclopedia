...
def classify(entry, cache, state):
    ...
    if best_tr >= 0.60 and (amatch or ymatch):
        ...
        result["note"] = "Moderate title match corroborated by author only — review (author matcher is load-bearing here)."
...
ORDER = ["mismatch", "not_found", "verified", "book_or_unindexed"]
LABELS = {
    "verified": "VERIFIED",
...
def write_report(results):
    ...
    # How each verified entry earned "verified" — the weak title+author path is called out.
    verified = [r for r in results if r["status"] == "verified"]
    weak = [r for r in verified if r["reason"] == "title0.45+author"]
    path_labels = {
        "doi_resolved": "resolved DOI (author/year/title consistent)",
        "title_ratio>=0.60": "high title ratio (>=0.60) + author/year",
        "title0.45+author": "MODERATE title (0.45–0.60) + author only — REVIEW",
    }
    counts = {}
    for r in verified:
        counts[r["reason"]] = counts.get(r["reason"], 0) + 1
    lines.append("## Verification path breakdown (of the {} verified)\n".format(len(verified)))
    ...
    lines.append("")
    lines.append("**Weak path (moderate title + author only, matcher is load-bearing): {}**\n".format(len(weak)))
    if weak:
        for r in weak:
            md = r["matched"] or {}
...
def main():
    ...
    for i, entry in enumerate(entries, 1):
        ...
        print("[{:2}/{}] {:17} {}{}".format(i, len(entries), r["status"], entry["id"], flag))
        if i % 10 == 0:
            save_cache(cache)
    save_cache(cache)
    ...
    weak_path = [r["id"] for r in verified if r["reason"] == "title0.45+author"]
    ...
    out = {
        ...
        "weak_path_verified_ids": weak_path,
    ...
    print("== weak-path (0.45+author) verified, for review ==", weak_path or "none")
